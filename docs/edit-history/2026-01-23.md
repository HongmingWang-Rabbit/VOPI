...
| 8 | Gemini Image Generation | Full Gemini Stack with native image generation, quality filtering, code review fixes |

---

## Session 8: Gemini Image Generation & Quality Filtering

### Overview

Implemented a "Full Gemini Stack" that uses Gemini for both video analysis AND image generation, replacing the traditional Stability/Claid/Photoroom pipeline for commercial image generation. Added AI-based quality filtering to remove low-quality generated images.
...
| File | Description |
|------|-------------|
| `src/providers/implementations/gemini-image-generate.provider.ts` | Gemini native image generation (white-studio + lifestyle variants) |
| `src/providers/implementations/gemini-quality-filter.provider.ts` | AI-powered quality filtering with reference image comparison |
| `src/providers/interfaces/gemini-image-generate.provider.ts` | Interface for image generation provider |
| `src/providers/interfaces/gemini-quality-filter.provider.ts` | Interface for quality filter provider |

#### Processors

| File | Description |
|------|-------------|
| `src/processors/impl/gemini/gemini-image-generate.ts` | Processor for generating commercial images via Gemini |
| `src/processors/impl/gemini/gemini-quality-filter.ts` | Processor for AI quality filtering of generated images |

#### Templates

| File | Description |
|------|-------------|
| `src/templates/gemini-image-generate-prompts.ts` | Prompt templates for white-studio and lifestyle variants |
| `src/templates/gemini-quality-filter-prompts.ts` | Prompt templates for quality evaluation |

#### Utilities

| File | Description |
|------|-------------|
| `src/utils/image-utils.ts` | Shared image utilities (MIME type, reference frame limiting) |
| `src/utils/frame-selection.ts` | Frame selection utilities (angle diversity, quality scoring) |

### Code Review Fixes Applied

#### Critical Fixes

1. **Safe imageId Delimiter** - Changed from `_` to `::` delimiter to avoid conflicts with frameIds like `frame_00001`
   ```typescript
   // Before: frame_00001_lifestyle (ambiguous)
   // After: frame_00001::lifestyle (unambiguous)
   ```

2. **Database WHERE Clause** - Added `localPath` matching for precise record updates
   ```typescript
   .where(
     and(
...
3. **s3Url Validation** - Added explicit null checks before type assertions with debug logging

4. **Reference Frame Limiting** - Limited to MAX_REFERENCE_FRAMES (4) to prevent API input limits

#### Code Quality Improvements

1. **Extracted Duplicated Code**:
   - `getImageMimeType()` → `src/utils/image-utils.ts`
   - `selectBestAngles()` → `src/utils/frame-selection.ts`
   - Prompts → `src/templates/gemini-*-prompts.ts`

2. **Replaced Magic Strings**:
   - Added `AGENT_FILTERED_FOLDER` constant
   - Added `IMAGE_ID_DELIMITER` constant

3. **Configurable Model**:
   - Added `AI_GEMINI_IMAGE_MODEL` config key
   - Added `geminiImageModel` to `EffectiveConfig`
...
|-----|---------|-------------|
| `ai.gemini_image_model` | `gemini-2.5-flash-image` | Model for Gemini image generation |

#### New Concurrency Key

| Key | Default | Description |
|-----|---------|-------------|
| `GEMINI_IMAGE_GENERATE` | 2 | Parallelism for image generation |

### Files Modified

| File | Changes |
|------|---------|
| `src/types/config.types.ts` | Added `AI_GEMINI_IMAGE_MODEL` key, `DEFAULT_GEMINI_IMAGE_MODEL`, `geminiImageModel` to EffectiveConfig |
| `src/services/global-config.service.ts` | Added `geminiImageModel` to getEffectiveConfig() |
| `src/processors/concurrency.ts` | Added `GEMINI_IMAGE_GENERATE` concurrency key |
| `src/processors/impl/index.ts` | Export new Gemini processors |
| `src/providers/implementations/index.ts` | Export new Gemini providers |
| `src/providers/interfaces/index.ts` | Export new provider interfaces |
| `src/processors/runner.test.ts` | Added `geminiImageModel` to test config |
| `src/processors/impl/util/filter-by-score.test.ts` | Added `geminiImageModel` to test config |

### Variant Types

The Gemini image generation supports two variants:

| Variant | Description | Prompt Strategy |
|---------|-------------|-----------------|
| `white-studio` | Product on pure white background | Framed as "background removal" task to preserve product fidelity |
| `lifestyle` | Product in contextual lifestyle setting | Creates appropriate scene while preserving exact product appearance |

### Quality Filter Criteria

The quality filter evaluates images against these criteria:

1. **Human Body Parts** (highest priority) - Rejects any visible hands, fingers, arms
2. **Product Mismatch** - Compares against reference images for text, patterns, colors
3. **Background Contamination** - For white-studio: no decorative elements in background
4. **Quality Issues** - Blurry, cropped product, awkward angles

### Architecture

```
Full Gemini Stack Flow:
download → gemini-unified-video-analyzer → gemini-image-generate → gemini-quality-filter → upload-frames → complete-job

Gemini Image Generation Flow:
1. Select best 4 angles from input frames
2. For each selected frame:
   - Generate white-studio variant (background removal)
   - Generate lifestyle variant (contextual scene)
3. Upload all generated images to S3 under commercial/
4. Quality filter evaluates against reference frames
5. Approved images copied to agent-filtered/ folder
```

### Test Results

```
All tests passing after adding geminiImageModel to test configurations
- TypeScript: No type errors
```

---

## Session 9: Credit Ledger System with Stripe Checkout

### Overview

Implemented a complete credit system for user-based billing, replacing API key usage tracking with a ledger-based credit balance. Users authenticate via OAuth, receive 5 free signup credits, and can purchase more via Stripe Checkout.

### New Database Tables

1. **`credit_transactions`** - Ledger for all credit changes
   - Tracks signup grants, purchases, spends, refunds, admin adjustments
   - Idempotency via unique `idempotency_key` column
   - Links to Stripe payment intent and checkout session IDs

2. **`stripe_events`** - Webhook idempotency tracking
   - Prevents double-processing of Stripe webhooks
   - Tracks event processing status and errors

3. **`signup_grants`** - Abuse prevention for free signup credits
   - One grant per user (unique constraint)
   - Tracks IP address and device fingerprint for abuse detection

4. **`users` table modifications**
   - Added `credits_balance` (cached balance)
...
| Method | Description |
|--------|-------------|
| `getBalance(userId)` | Get cached balance from users table |
| `getBalanceWithHistory(userId, limit)` | Balance + recent transactions |
| `hasReceivedSignupGrant(userId)` | Check if user got free credits |
| `checkSignupAbuse(ip, device)` | Validate IP/device limits |
| `grantSignupCredits(userId, email, ip, device)` | Grant 5 credits (idempotent) |
| `addPurchasedCredits(userId, pack, stripeEventId, ...)` | Add from Stripe (idempotent) |
| `spendCredits(userId, amount, idempotencyKey, jobId)` | Deduct credits (idempotent) |
| `recalculateBalance(userId)` | Fix cached balance from ledger |
| `calculateJobCost(request)` | Calculate job cost based on video duration |
| `calculateJobCostWithAffordability(userId, request)` | Cost + can afford check |

#### `src/services/stripe.service.ts`

| Method | Description |
|--------|-------------|
| `isConfigured()` | Check Stripe env vars |
| `getOrCreateCustomer(userId, email)` | Optimistic customer creation |
| `createCheckoutSession(...)` | Create Stripe checkout session |
| `verifyWebhookSignature(payload, sig)` | Validate webhook signature |
| `isEventProcessed(eventId)` | Check idempotency |
| `processWebhookEvent(event)` | Handle checkout.session.completed |

### New API Endpoints

| Endpoint | Auth | Description |
|----------|------|-------------|
| `GET /api/v1/credits/balance` | JWT | Get balance + transactions |
| `GET /api/v1/credits/packs` | None | Get available credit packs |
| `GET /api/v1/credits/pricing` | None | Get pricing configuration |
| `POST /api/v1/credits/estimate` | Optional | Estimate job cost |
| `POST /api/v1/credits/checkout` | JWT | Create Stripe checkout session |
| `POST /api/v1/credits/webhook` | Stripe sig | Handle Stripe webhooks |
| `POST /api/v1/credits/spend` | JWT | Spend credits (idempotent) |
| `POST /api/v1/credits/recalculate` | Admin | Recalculate cached balance |

### Credit Packs

| Pack | Credits | Price |
|------|---------|-------|
| Free signup | 5 | $0 (once per user) |
| Single | 1 | $0.99 |
| Pack 20 | 20 | $14.99 |
| Pack 100 | 100 | $59 |
| Pack 500 | 500 | $199 |
...
Job costs are calculated based on:
- Base cost (default: 1 credit)
- Duration cost (default: 0.05 credits/second)
- Included frames (default: 4)
- Extra frame cost (default: 0.25 credits/frame)
- Add-on services (commercial video)
- Min/max job cost limits
...
STRIPE_PRICE_ID_CREDIT_1=price_...
STRIPE_PRICE_ID_PACK_20=price_...
...
Added `pnpm db:seed` command to seed:
- Default pricing configuration in `global_config`
- Optional test user with credits (via `TEST_USER_EMAIL` env var)

### Tests Added

| File | Tests | Description |
|------|-------|-------------|
| `src/services/credits.service.test.ts` | 25 | Credit operations |
| `src/services/stripe.service.test.ts` | 28 | Stripe integration |
| `src/processors/impl/credits/spend-credits.test.ts` | 10 | Spend processor |

**Total new tests**: 63

### Pipeline Integration

Added `spend-credits` processor to the pipeline:
- Calculates job cost based on video duration
- Deducts credits before processing
- Returns structured error with cost breakdown on insufficient credits
- Stores credit info in job metadata

### Files Created

| File | Description |
|------|-------------|
| `src/types/credits.types.ts` | Credit types, packs, pricing config, Zod schemas |
| `src/services/credits.service.ts` | Credit ledger operations |
| `src/services/credits.service.test.ts` | Credit service tests |
| `src/services/stripe.service.ts` | Stripe checkout/webhook handling |
| `src/services/stripe.service.test.ts` | Stripe service tests |
| `src/routes/credits.routes.ts` | API endpoints |
| `src/processors/impl/credits/spend-credits.ts` | Credit spend processor |
| `src/processors/impl/credits/spend-credits.test.ts` | Spend processor tests |
| `src/db/seed-credits.ts` | Database seed script |
| `src/db/migrations/0005_add_credit_ledger.sql` | Migration for credit tables |

### Files Modified

| File | Changes |
|------|---------|
| `src/db/schema.ts` | Added credit tables and user columns |
| `src/config/env.ts` | Added Stripe env schema |
| `src/config/index.ts` | Added stripe config section |
| `src/app.ts` | Registered credits routes, raw body parsing for webhook |
| `package.json` | Added db:seed script and stripe dependency |
| `.env.example` | Added Stripe env vars |

### Code Review Fixes Applied

1. **Stripe API outside transaction** - Refactored `getOrCreateCustomer` to use optimistic approach instead of holding database locks during Stripe API calls
2. **Unvalidated type cast** - Added Zod schema validation for recalculate endpoint body
3. **Removed unused imports** - Cleaned up unused `sql` import in stripe.service.ts

### Test Results (Final)

```
All 1170 tests passing
- Credit service tests: 25 passing
- Stripe service tests: 28 passing
- Spend processor tests: 10 passing
```
